...
public class EntityFactory {
    ...
    public Mercenary buildAssassin(Position pos) {
        ...
        int assassinBribeAmount = config.optInt("assassin_bribe_amount", Assassin.ASSASSIN_BRIBE_AMOUNT);
        int assassinBribeRadius = config.optInt("bribe_radius", Assassin.DEFAULT_BRIBE_RADIUS);
        double assassinBribeFailRate = config.optDouble("assassin_bribe_fail_rate", Assassin.ASSASSIN_BRIBE_FAIL_RATE);
    ...
    public Sceptre buildSceptre() {
        int mindControlDuration = config.optInt("mind_control_duration", Sceptre.MIND_CONTROL_DURATION);
        return new Sceptre(mindControlDuration);
    }
    public MidnightArmour buildMidnightArmour() {
        int midnightArmourAttack = config.optInt("midnight_armour_attack", MidnightArmour.MIDNIGHT_ARMOUR_ATTACK);
        int midnightArmourHealth = config.optInt("midnight_armour_health", MidnightArmour.MIDNIGHT_ARMOUR_DEFENCE);
        return new MidnightArmour(0, midnightArmourAttack, midnightArmourHealth);
    }
    ...
    private Entity constructEntity(JSONObject jsonEntity, JSONObject config) {
        ...
        case "swamp_tile":
            int stuckTime = config.optInt("movement_factor", 1);
            return new SwampTile(pos, stuckTime);
        default:
            return null;
        }
    }
