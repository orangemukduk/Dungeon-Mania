...
public class MercenaryTest {
    ...
    @Test
    @DisplayName("Testing allied mercenary goes around wall to reach player")
    public void allyDjikstraPathWithWalls() {
        DungeonManiaController dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame("d_MercenaryMovement", "c_MercenaryMovementTest");

        String mercId = TestUtils.getEntitiesStream(res, "mercenary").findFirst().get().getId();
        // pick up first treasure
        res = dmc.tick(Direction.RIGHT);
        // Movement should follow djikstra as it is enemy
        assertEquals(1, TestUtils.getInventory(res, "treasure").size());
        assertTrue(new Position(6, 0).equals(getMercPos(res))
            || new Position(6, 2).equals(getMercPos(res)));

        // attempt bribe
        res = assertDoesNotThrow(() -> dmc.interact(mercId));
        assertEquals(0, TestUtils.getInventory(res, "treasure").size());

        // Check Allied Mercenary position -> confirm it is following the same patterna s ally
        res = dmc.tick(Direction.LEFT);

        assertTrue(new Position(5, 3).equals(getMercPos(res))
            || new Position(5, -1).equals(getMercPos(res)));

        // Check Allied Mercenary position -> confirm it is following the same patterna s ally
        res = dmc.tick(Direction.RIGHT);
        assertTrue(new Position(4, 3).equals(getMercPos(res))
            || new Position(4, -1).equals(getMercPos(res)));

        // Check Allied Mercenary position -> confirm it is following the same patterna s ally
        res = dmc.tick(Direction.LEFT);
        assertTrue(new Position(3, 3).equals(getMercPos(res))
            || new Position(3, -1).equals(getMercPos(res)));

        // Check Allied Mercenary position -> confirm it is following the same patterna s ally
        res = dmc.tick(Direction.RIGHT);
        assertTrue(new Position(3, 2).equals(getMercPos(res))
            || new Position(3, 0).equals(getMercPos(res)));
        // Check the position to make sure it is following the player
        res = dmc.tick(Direction.LEFT);
        assertTrue(new Position(2, 2).equals(getMercPos(res))
            || new Position(2, 0).equals(getMercPos(res)));


        // now that the player is adjacent to the ally it should follow the players previous position
        // which was the position at the beginning of the game
        res = dmc.tick(Direction.RIGHT);
        //assertTrue(isAdjacent(getMercPos(res), TestUtils.getPlayerPos(res)));

        assertTrue(new Position(1, 1).equals(getMercPos(res)));
    }

    @Test
    @DisplayName("Testing allied mercenary stays dormant when surrounded by walls")
    public void allyStationary() {
        DungeonManiaController dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame("d_MercenaryMovement1", "c_MercenaryMovementTest");
        ...
        // Movement should follow djikstra as it is enemy
        assertEquals(1, TestUtils.getInventory(res, "treasure").size());
        assertTrue(new Position(6, 1).equals(getMercPos(res)));
